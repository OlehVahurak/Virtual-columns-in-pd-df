...
def test_empty_result_when_invalid_rules():
    ...
    assert df_result.empty, (
        "Should return an empty df when the role has invalid character: '\\'.\n\n"
        f"Result:\n\n{df_result}\n\nExpected:\n\nEmpty df"
    )
    ...
    df_result = add_virtual_column(
        df,
        "label_five + label_two",
        "label_three"
    )

    assert df_result.empty, (
        "Should return an empty df when the role has a column which isn't in the df: "
        "'label_five'.\n\n"
        f"Result:\n\n{df_result}\n\nExpected:\n\nEmpty df"
    )
...
def test_when_extra_spaces_in_rules():
    ...
    assert df_result.equals(df_expected), (
        "Should work when the rule hasn't spaces between the operation and the column.\n\n"
        f"Result:\n\n{df_result}\n\nExpected:\n\n{df_expected}"
    )
    ...
    assert df_result.equals(df_expected), (
        "Should work when the rule has spaces between the operation and the column.\n\n"
        f"Result:\n\n{df_result}\n\nExpected:\n\n{df_expected}"
    )
    ...
    assert df_result.equals(df_expected), (
        "Should work when the rule has extra spaces at the start/end.\n\n"
        f"Result:\n\n{df_result}\n\nExpected:\n\n{df_expected}"
    )
